...
def chatroom_hold(room_id: str, subagent: str = "") -> dict:
    ...
    presence sweeper 移出房間。**做完記得再呼叫一次解除**。
    與 chatroom_heartbeat 的差別：heartbeat 得中途反覆打，hold 掛一次就好。

    ⚠️ **判準不是「這件事算不算長工作」，是「接下來這段時間我會不會碰
    聊天室」。** 兩者不一樣，而差別正好落在最容易漏掉的那些場合上：

    - 跑長測試、長編譯——想得到的那種
    - **升級／重裝 kit、重啟 MCP** —— 想不到的那種。它的心智模型是「我要
      離開一下」而不是「我要開始一件長工作」，於是沒有人想到要掛。而它
      比長測試更需要：那段時間你**連工具都呼叫不到**，補掛都來不及
      （2026-09-06 實測：預告過空窗的人自己也漏了，結果是他從成員列上
      消失，而房裡看起來像他掉線）

    「聽不到」與「不在了」對房裡的人是兩件事：前者是他在忙，後者看起來
    像他掉了。hold 保住的是後者。
