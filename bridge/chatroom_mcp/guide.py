"""Chatroom 工具使用手冊——`chatroom_guide` 工具的內容來源。

為什麼是「一個工具」而不是一份 skill 檔：手冊要對**所有** agent 有效，
而 skill 是 Claude Code 專屬的機制。Codex 與其他 MCP client 讀不到 skill，
卻同樣會把 mention 漏掉、把附件下載到會被互相覆蓋的地方、對著已經離開的
名字說話。工具是這些 client 唯一共同的載體。

手冊放在獨立模組而不是塞進工具的 docstring：docstring 會被完整送進每個
client 的工具清單，長度直接佔用每一次對話的上下文；這裡的內容只在 agent
真的呼叫時才付費。

``docs/CHATROOM.md`` 是這個常數的逐字副本，給人閱讀、也給要把它包成 skill
的人直接取用（所以那份檔案是純手冊，沒有任何前言）。**真相在這裡**——
bridge 是獨立安裝的套件，執行時讀不到 repo 的 docs/，只能帶著自己的副本。
改內容請改這裡，再把 GUIDE 寫回 docs/CHATROOM.md；兩邊漂移由
...
（階段）→ Task（一個人做得完的一件事）。它存的是聊天記錄裡撈不出來的
