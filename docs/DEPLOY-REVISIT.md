...
| 你的情況 | 走哪條 | 代價 |
|---|---|---|
| 成員都在同一個 VPN（Radmin / Tailscale）或同區網 | **不要開隧道**，直接發內網位址 | 無。這是最安全也最穩的路 |
...
3. **Cloudflare Access 未做**（§2 #4）——目前靠 README 的自律段落擋著。
   要不要做是產品決定，不是技術決定：做了就多一層登入，對「快速上手」是負擔。
   **建議維持不做，但把它從「沒提過」變成「明知而不做」**——也就是這一行。
