...
（決策 2026-09-08，起因是有人拿這條規則去看 directive 的多房投遞）

這條規則**只適用於有保底留痕的通知類**。指令類反過來，以投遞保證優先：

| | `board_task_done`（通知） | `board_directive`（指令） |
|---|---|---|
| 有沒有保底 | **有**：來源房那則留痕永遠在 | **沒有**，它只有那一則 |
| 性質 | 事後知道就好 | **等他行動** |
| 漏送的代價 | 他晚點從板上看到 | **永遠不會醒，而送出端看不出來** |
| 多房投遞 | 擇一（投過就從名單移除） | **每一間都投** |

⇒ **directive 對同一個人重複喚醒是刻意付的保險費，不是缺陷。**
判準是 2026-09-02 T5-4 定的：「去重要去的是『同一個人被通知多次』，不是
『同一個人只在其中一個房被通知』——去重的單位是**房**不是**人**」。
`test_directive_reaches_every_room_the_target_is_in` 釘著它。

根本困難：**Hub 不知道 agent 在哪一間房掛了 watcher。** 他在 A、B 都有身分
卻只監看 B 時，只投 A 就是靜默失效。要真正消掉這筆保險費，得先讓 Hub 知道
每個 actor 的活躍 long-poll 在哪些房——那是新功能，不是修 bug。
