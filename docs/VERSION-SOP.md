...
### 沒有出口的等待，等於一個永遠不會回傳結論的檢查

**任何 `until <條件>` 的等待迴圈都要有「條件永遠不成立」的出口**——次數上限
或時間上限，到了就報「等不到」，而不是繼續等。

2026-09-05 找到一支 `watch_restart.sh` **跑了兩天沒停**：每 30 秒輪詢一塊板，
等回應裡出現 `visibility` 欄位。那塊板零掛接房 ⇒ 非 owner 一律 403 ⇒
**那個欄位永遠不會出現**。它不是在等一件還沒發生的事，是在等一件不會發生的事。

三件事疊起來才讓它活那麼久：

1. **等待迴圈的失敗長成「還在等」**——沒有任何一刻它會說自己壞了
2. **session 結束不會帶走它**（「斷開 ≠ 進程退場」，同一天的第三個版本）
3. **它輪詢的是別人的機器**——代價出現在對方的 access log 裡，兩天後才被看見

與「離開碼是唯一機器可讀結論」是同一件事的兩面。

### 寫探針之前先知道的兩件事

**① `board_id` 是 lazy 生成的：新房要建過第一張卡，它才存在。**
在那之前拿到的是 `null`，於是後續每一支都打到 `/api/boards/None` ⇒ 一連串
404 ⇒ **看起來像「這組功能整個壞了」**。⇒ 順序是**先建一張卡，再取
`board_id`**。（`null` ＝「還沒建」不是「出錯了」——又一次把「還沒有」
折進「沒有」。）

**② 同名欄位在不同層級是不同的數。** 想法板有 `rev`，段落也有 `rev`。
拿錯的症狀是**每一發都 409**，而那看起來像端點壞了、不像參數拿錯。

> 409 回應裡的 `your_rev` 與 `rev` 就是給你比對用的——**被連續拒絕時先看它們**，
> 兩個數字擺在一起，答案就在臉上。
...
- **token 不要以檔案形式留在 scratchpad。** 同一次事故裡那支腳本 `source`
  了暫存目錄的 `tk.sh`，**token 明碼躺了兩天**。不是外洩，但暫存目錄的壽命
  比你以為的長——那支腳本本身就是證據。
