...
### J-7 版本與診斷（`ce8a2f3` / `1185e91` / logging）

- [ ] J29 設定畫面測試連線顯示的 hub 版本是 **1.2.0**，不是 1.1.5
- [ ] J30 三包版號一致（server / bridge / app）
- [ ] J31 Hub 啟動時，**主控台**（不是只有 `hub.jsonl`）看得到
      「自動清理已啟用……會被永久刪除」那則 WARNING
      ❌ 壞掉的樣子：只有 jsonl 有 → 一個為人設計的預告，人看不到
- [ ] J32 INFO 不上主控台（噪音等於沒有警告），且 `hub.jsonl` 那份不受影響
- [ ] J33 沒帶 `CHATROOM_DEBUG_ENDPOINTS` 時 `/api/_debug/boom` 是 **404**，
      帶不帶 token 都一樣（關著時連「這裡有東西」都不說）
