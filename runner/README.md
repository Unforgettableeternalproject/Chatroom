...
| `allowed_mcp_servers` | run 准用的 MCP 伺服器（預設 `["chatroom"]`）。**預設拒絕**：自檢 `claude mcp list` 看得到的每一台（跟著登入進來的 claude.ai 連接器，以及執行器設定目錄註冊的本機 stdio 伺服器），不在這裡也沒被 `extra_allowed_tools` 的 `mcp__<server>__*` 點名的，一律寫進 run 專用 settings 的 `deniedMcpServers`（伺服器不載入）與 `--disallowedTools`／`permissions.deny`（工具移出 context）。App 的「這台機器 → 執行器」分頁有勾選介面。名字也吃使用者全域 `~/.claude.json`（`CLAUDE_CONFIG_DIR` 有設就先看那裡）的 `mcpServers`——fff、mempal 這種自訂伺服器不在執行器的設定目錄裡，點名之後定義會**原樣複製**進每一筆 run 的 `mcp.json`（`env` 不展開）；全域檔不存在或壞掉只記 warning，不擋 run |
