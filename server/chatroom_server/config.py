...
class Config:
    ...
    #
    # 🚨 **這裡的 `3` 已經被推翻過，不要照著它推論現況。**
    #
    # 2026-08-30 選 3 天時，「三天後會發生什麼」還沒有人實際看過。09/06 看到了：
    # 一間 633 則訊息的房到期被永久刪除。艾斯維爾在資訊補齊之後重裁
    # **15 天**，正式 Hub 以 `CHATROOM_PURGE_ARCHIVED_DAYS=15` 跑。
    #
    # 預設值當天**刻意沒跟著改**——那是行為改變（影響每一台新起的 Hub），
    # 而收尾時不塞沒把握的行為改變。「預設要不要跟進 15」列為下輪待裁。
    #
    # 所以現在有兩個數字：**環境變數 15（現行）、這裡 3（未定）**。
    # 寫下這段的理由就是它們不一致：下一個人只讀得到 `config.py`——別人機器上
    # 的環境變數他看不見——而一個沒有理由的 `3` 會被當成現行決定。
    #
    # ⚠️ 若下輪裁定「預設留 3、每台自己帶環境變數」，**這段更要留著**：
    # 那時 `3` 旁邊必須寫著它被推翻過，否則同一個誤讀會再發生一次。
