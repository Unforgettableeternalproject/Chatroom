...
class Config:
    ...
    #
    # 🚨 **這個 15 是被推翻過一次才來的，不要當成隨手選的數字。**
    #
    # 2026-08-30 原本選 3 天，而那時「三天後會發生什麼」還沒有人實際看過。
    # 09/06 看到了：一間 **633 則訊息**的房到期被永久刪除。艾斯維爾在資訊
    # 補齊之後重裁 **15 天**，同日跟進成預設值。
    #
    # ⚠️ 決定原本只活在正式 Hub 的環境變數裡，預設仍是 3——那表示**下一台
    # 新起的 Hub、或一次漏帶變數的重啟，都會安靜地退回 3 天**。
    # 「決定寫了、但沒寫在會被讀到的地方」是這個專案反覆踩到的那一族：
    # 下一個人只讀得到 `config.py`，別人機器上的環境變數他看不見。
    #
    # 環境變數仍然覆寫得掉（測試 Hub 設 0 關掉自動清理是常見用法）——
    # 改的是預設，不是把它寫死。`tests/test_room_deletion.py` 兩條釘住這兩半。
    purge_archived_days: float = field(
        default_factory=lambda: float(os.environ.get("CHATROOM_PURGE_ARCHIVED_DAYS", "15"))
    )
